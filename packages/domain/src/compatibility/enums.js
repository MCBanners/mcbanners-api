...
export const bannerTypeValues = bannerTypeRecords.map((record) => record.name);
export const bannerTypeSchema = z.enum(bannerTypeValues);
export const bannerTypeOrdinalMap = Object.freeze(Object.fromEntries(bannerTypeRecords.map((record) => [record.ordinal, record.name])));
export const bannerTypeByName = Object.freeze(Object.fromEntries(bannerTypeRecords.map((record) => [record.name, record])));
export const bannerOutputFormats = [
    { enumName: "PNG", value: "png" },
    { enumName: "JPEG", value: "jpg" }
];
export const bannerOutputFormatValues = bannerOutputFormats.map((format) => format.value);
export const bannerOutputFormatSchema = z.enum(bannerOutputFormatValues);
export const textThemeValues = ["LIGHT", "DARK"];
...
export const backgroundTemplateValues = backgroundTemplateRecords.map((record) => record.name);
export const backgroundTemplateSchema = z.enum(backgroundTemplateValues);
export const backgroundTemplateByName = Object.freeze(Object.fromEntries(backgroundTemplateRecords.map((record) => [record.name, record])));
...
export const decodeBannerTypeOrdinal = (ordinal) => bannerTypeOrdinalMap[ordinal];
export const getBackgroundTemplateFileName = (template) => `${template.toLowerCase()}.png`;
export const getBackgroundTemplateTextTheme = (template) => backgroundTemplateByName[template].textTheme;
export const getFontFaceFileName = (fontFace, bold = false) => {
...
export const parseBannerOutputFormat = (raw) => {
