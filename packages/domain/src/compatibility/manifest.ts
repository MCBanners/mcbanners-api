import type { CompatibilityManifest, SettingNamespace } from "./schema";

const background: SettingNamespace = {
  name: "background",
  kind: "background",
  defaults: { template: "MOONLIGHT_PURPLE" }
};

const logo: SettingNamespace = {
  name: "logo",
...
const text = (
  name: string,
  overrides: Partial<Record<keyof typeof textDefaults | "x" | "y", string | number | boolean>>
): SettingNamespace => ({
  name,
  kind: "text",
...
const base = [background, logo] as const;
...
export const compatibilityManifest = {
  ...
  routes: [
    ...
    },
    {
      ...
      legacySource: "banner-api",
      behavior:
        "Return namespaced default settings for author/resource/member/team/discord/server/all."
    },
    {
      method: "GET",
  ...
  discordCommands: [
    {
      ...
      urlTemplate:
        "https://api.mcbanners.com/banner/{type}/{platform}/{id}/banner.png?template={TEMPLATE}"
    },
    {
      command: "/server icon host [port]",
      behavior:
        "Defers, calls /mc/icon with default port 25565, replies with URL on 200 or 'Server not found!'.",
      urlTemplate: "https://api.mcbanners.com/mc/icon?host={host}&port={port|25565}"
    },
    {
      command: "/server info host [port]",
      behavior:
        "Defers, calls /mc/server with default port 25565, sends an embed with host/version/players/MOTD and icon thumbnail.",
