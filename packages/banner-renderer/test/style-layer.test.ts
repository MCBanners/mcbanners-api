...
describe("parseBannerStyleSettings", () => {
  ...
  test("infers v1 from explicit style field without style__version", () => {
    const result = parseBannerStyleSettings({ "background__mode": "solid", "background__color": "ffffff" });
    expect(result).not.toBeNull();
    expect(result!.background.mode).toBe("solid");
  });
...
describe("validateBannerStyleSettings", () => {
  ...
  test("rejects invalid hex color for background__color", () => {
    ...
    expect(errors.some((e) => e.code === "INVALID_HEX_COLOR" && e.field === "background__color")).toBe(true);
  });

  test("rejects invalid hex for text colors", () => {
    const errorsP = validateBannerStyleSettings({ "text__primary_color": "white" });
    expect(errorsP.some((e) => e.code === "INVALID_HEX_COLOR" && e.field === "text__primary_color")).toBe(true);

    const errorsS = validateBannerStyleSettings({ "text__secondary_color": "rgb(0,0,0)" });
    expect(errorsS.some((e) => e.code === "INVALID_HEX_COLOR" && e.field === "text__secondary_color")).toBe(true);

    const errorsA = validateBannerStyleSettings({ "text__accent_color": "#rrggbb" });
    expect(errorsA.some((e) => e.code === "INVALID_HEX_COLOR" && e.field === "text__accent_color")).toBe(true);
  });
...
describe("buildResourceBannerNodes — solid background", () => {
  ...
  test("first node is a fill-rect node", () => {
    const nodes = buildResourceBannerNodes(FIXTURE_SPIGOT_FREE, DEFAULT_RESOURCE_BANNER_SETTINGS, style);
...
describe("buildResourceBannerNodes — text colors", () => {
  ...
  test("missing text color fields fall back to default color behavior", () => {
    const nodes = buildResourceBannerNodes(FIXTURE_SPIGOT_FREE, DEFAULT_RESOURCE_BANNER_SETTINGS);
    const noStyleNodes = buildResourceBannerNodes(FIXTURE_SPIGOT_FREE, DEFAULT_RESOURCE_BANNER_SETTINGS, null);
...
describe("buildResourceBannerNodes — logo Y offset", () => {
  ...
  test("logo__y=0 preserves auto-centered position", () => {
    const nodes = buildResourceBannerNodes(FIXTURE_SPIGOT_FREE, DEFAULT_RESOURCE_BANNER_SETTINGS, bgStyle("0"));
  ...
  test("positive logo__y shifts logo down", () => {
    const nodes = buildResourceBannerNodes(FIXTURE_SPIGOT_FREE, DEFAULT_RESOURCE_BANNER_SETTINGS, bgStyle("10"));
    const logo = nodes.find((n) => n.type === "image" || n.type === "sprite")!;
    expect(logo.y).toBe(baseY + 10);
  });

  test("negative logo__y shifts logo up", () => {
    const nodes = buildResourceBannerNodes(FIXTURE_SPIGOT_FREE, DEFAULT_RESOURCE_BANNER_SETTINGS, bgStyle("-5"));
    const logo = nodes.find((n) => n.type === "image" || n.type === "sprite")!;
...
describe("saved-banner compatibility", () => {
  test("null style produces same output as no style argument", () => {
    const noStyleNodes = buildResourceBannerNodes(FIXTURE_SPIGOT_FREE, DEFAULT_RESOURCE_BANNER_SETTINGS);
    const nullStyleNodes = buildResourceBannerNodes(FIXTURE_SPIGOT_FREE, DEFAULT_RESOURCE_BANNER_SETTINGS, null);
