...
const findBreakBefore = (line, start) => {
    for (let i = start; i >= 0; i--) {
        const c = line[i];
        if (c !== undefined && (/\s/u.test(c) || c === "-"))
            return i;
    }
    return -1;
};
/** Scans forward from `start` for a whitespace or hyphen break point. */
const findBreakAfter = (line, start) => {
 ...
 */
const wrapLineInto = (line, result, measurer, maxWidth) => {
    let remaining = line;
 ...
 */
export const wrapText = (measurer, text, maxWidth) => {
