...
const drawAligned = (surface, text, x, y, align) => {
 ...
 */
export const renderTextNode = (surface, node) => {
 ...
 */
export const renderWrappedTextNode = (surface, node) => {
    surface.context.font = buildFontSpec(node.fontFace, node.fontWeight, node.fontSize);
    surface.context.fillStyle = rgbaColorToString(node.color);
    const text = node.maxChars !== undefined ? truncateText(node.content, node.maxChars) : node.content;
    const measurer = (t) => surface.context.measureText(t).width;
